...
// import {getAccount, getProducts, getOrders, getCartSuppliers, orderResponse} from './apis'
const apis = require('./nodeapis')
//PLEASE NOTE, VARIABLE "body" REFERS TO TO RESPONSE BODY OF TESTED API CALL

const errors = [];

const testScript = async (errors) => {
    
    //1. GET ACCOUNT
    let accountById = {} //get account to use in next step
    const idQuery = {id: 'arvindsdeli'}
    const codeQuery = {code: '12345'}
    //1A. Get Account by Id - SUCCESS CODE: accountById.id === idQuery.id 
    
    try {    
    accountById = await apis.getAccount({query: idQuery})
    if (accountById.id !== idQuery.id)
        errors.push({test: 'Get Account By Id', endpoint:'getAccount', query: idQuery, failure: 'wrong account returned'})                
    }
    catch (error) {
        errors.push({test: 'Get Account By Id', endpoint:'getAccount', query: idQuery, failure: error})
        
    }

    //1B. Get Account by Code  SUCCESS CODE: accountByCode.code === codeQuery.code
    try {    
        const accountByCode = await apis.getAccount({query: codeQuery})  
    if (accountByCode.code !== codeQuery.code)
        errors.push({test: 'Get Account By Code', endpoint:'getAccount', query: idQuery, failure: 'wrong account returned'})                
    }
    catch (error) {
        errors.push({test: 'Get Account By Code', endpoint:'getAccount', query: idQuery, failure: error})
    }

    //2. GET PRODUCTS
    //2A. Get Products by SKU query - SUCCESS: all sku properties in array of objects are in skuList in exact same order & have prices
    let orderGuide = []
    let accountId = ''
    try {
        orderGuide = accountById.orderGuide //from previous module
        accountId = accountById.id
    }
    catch {
        orderGuide = ['sysco-100', 'woolco-150', 'aceendico-110']
        accountId = idQuery.id
    }

    try {
    let initialFilter = [{ 'field': 'sku', 'comparison': ':', 'values': orderGuide }]
    let productList = await apis.getProducts({accountId: accountId, initialFilter: initialFilter, sort: {}, search: '', filter: []})
    for (var i = 0; i < orderGuide.length; i++)
        if (productList[i].sku !== orderGuide[i]) { //make sure SKUS are returned in same way
            errors.push({test: 'Get Products By SkuList', endpoint:'getProducts', query: orderGuide, failure: 'Did Not return correct SKU list'})
            break;
        }     
    } catch (error) {
        errors.push({test: 'Get Products By SkuList', endpoint:'getProducts', query: orderGuide, failure: error})
    }    
...
testScript(errors).then(errors => errors.length !== 0 ? console.log(errors) : console.log('FULL TEST SUCCESS'))
...
// ]
// const search = 'chicken'
// let prod = await apis.getProducts({accountId: accountId, initialFilter: [], sort: sort, search: search, filter: filter})
...
// let supplierList = await apis.getCartSuppliers({suppliers: suppliers})
...
// const orderResponse = await apis.placeOrder({supplierOrder: supplierOrder})
...
// //SUCCESS 5:
// //checkOrder = await apis.getOrders({query: {id: body.orderId}})[0]
...
// //6. SET ORDER DETAILS
// //6A. Set order detail
// const orderset = await apis.setOrder({id: orderId, update:{status: 'Delivered'}})

// //SUCCESS: call same order - status should be set as delivered
// //SUCCES CODE:
// // const getSetOrder = await apis.getOrders({query: {id: orderId}})
